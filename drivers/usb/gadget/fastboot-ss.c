...
int fboot_usb_int_bulkin(const char *buffer, unsigned int buffer_size)
{
	...
	g_pBulkInTrb = (usbdev3_trb_ptr_t)exynos_usb_malloc(sizeof(usbdev3_trb_t), USBDEV3_MDWIDTH/8);
	exynos_usb_fill_trb(g_pBulkInTrb, (u32)buffer, buffer_size, usbdev3_trb_ctrl.data, 1);
...
int fastboot_init(struct cmd_fastboot_interface *interface)
{
	...
	fastboot_interface->nand_block_size               = CFG_FASTBOOT_PAGESIZE * 64;//128k
	fastboot_interface->transfer_buffer               = (unsigned char *) CFG_FASTBOOT_TRANSFER_BUFFER;//0x48000000
	fastboot_interface->transfer_buffer_size          = CFG_FASTBOOT_TRANSFER_BUFFER_SIZE; //384M
